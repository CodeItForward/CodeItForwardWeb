...
const Header = () => {
  ...
  return (
    <header className="bg-transparent fixed w-full top-0 z-[9999]">
      ...
      {/* Mobile menu */}
      <div 
        className={`${isMenuOpen ? 'block' : 'hidden'} md:hidden absolute right-4 top-16 bg-blue-700 shadow-lg z-[9999] rounded-xl overflow-hidden mt-2 w-64`} 
        style={{ 
        ...
        <div style={{ marginTop: '4px', marginBottom: '4px' }}>
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="text-white hover:bg-blue-800 block rounded-lg text-base font-medium transition-colors duration-200"
              style={{ 
                padding: '10px', 
                marginBottom: '8px',
                display: 'block'
